...
mxArray* __new_mxArray(int _ndim, int *_shape, size_t _sz, mxtype _mt)
{
    ...
    cout << "OMPCpy: Allocated " << sz*_sz << " bytes for " << _ndim << " dims " << ret << "!" << endl;
...
mxArray* mxCreateNumericArray(int ndims, int* dims, mxClassID clsid, mxComplexity ComplexFlag)
{
    ...
    if (ComplexFlag == mxREAL)
        ret = __new_mxArray(ndims, dims, newsz, mxtype_FLOAT64);
    else if (ComplexFlag == mxCOMPLEX)
        ret = __new_mxArray(2, dims, 2*newsz, mxtype_FLOAT64);
